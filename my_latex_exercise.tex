...
\begin{MyAnswer}[ref={ex:logarithm2}]
        ...
        \Question \mybox{答案为 $x = 2$;}\\ 解：根据对数运算法则 $\log_a M + \log_a N = \log_a (MN)$, 则 $\log_2 (2x - 3) + \log_2 (x + 1) = \log_2 [(2x - 3)(x + 1)]$. 已知 $\log_2 [(2x - 3)(x + 1)] = 2$, 即 $(2x - 3)(x + 1) = 2^2 = 4$. 展开得 $2x^2 - x - 3 = 4$, 即 $2x^2 - x - 7 = 0$. 解得 $x = 2$ 或 $x = -\frac{7}{4}$. 经检验, 当 $x = 2$ 时, $2x - 3 = 1 > 0$ 且 $x + 1 = 3 > 0$, 满足对数函数的定义域要求; 当 $x = -\frac{7}{4}$ 时, $2x - 3 = -\frac{13}{2} < 0$, 不满足定义域要求. 所以 $x = 2$.
